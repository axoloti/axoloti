...
public class Midi extends gentools {
    ...
    static AxoObject Create_keyb_mod() {
        ...
        o.sMidiCode = "if ((status == MIDI_NOTE_ON + %midichannel%) && (data2)) {\n"
                ...
                + "  _gate = 1<<27;\n"
                + "  _gate2 = 0;\n"
                + "  PExModulationSourceChange(&parent->PExModulationSources[MODULATOR_%name%_velocity][0],NMODULATIONTARGETS,_velo<<20);\n"
                + "  PExModulationSourceChange(&parent->PExModulationSources[MODULATOR_%name%_note][0],NMODULATIONTARGETS,_note<<20);\n"
                + "} else if (((status == MIDI_NOTE_ON + %midichannel%) && (!data2))||\n"
                + "          (status == MIDI_NOTE_OFF + %midichannel%)) {\n"
                + "  if (_note == data1-64) {\n"
                + "    _rvelo = data2;\n"
                + "    _gate = 0;\n"
                + "  PExModulationSourceChange(&parent->PExModulationSources[MODULATOR_%name%_releasevelocity][0],NMODULATIONTARGETS,_rvelo<<20);\n"
                + "  }\n"
                + "} else if ((status == %midichannel% + MIDI_CONTROL_CHANGE)&&(data1 == MIDI_C_ALL_NOTES_OFF)) {\n"
                + "  _gate = 0;\n"
                + "}\n";
